...
class Supervisor {
    ...
    public static function punchCard(Employee $employee, Carbon $time ,$log = null)
    {
        ...
        if (self::isCheckingIn($lastCard)) {
            ...
            return self::checkIn($employee, $time, $lastCard ,$log);
    ...
    private static function validateCheckOut(Employee $employee, $lastCard, Carbon $time, $log)
    {
        ...
        if($log->InOutType == TA_LOG::CHECKIN){

            return self::checkIn($employee, $time, $lastCard, $log);

        }
        else if($log->InOutType == TA_LOG::CHECKOUT){
     ...
     */
    private static function checkIn(Employee $employee, Carbon $time, $lastCard, $log=null)
    {
    ...
    private static function getClockedShiftMinutes($shift, $attendance, $log = null, $checkout =false)
    {
        ...
        $shiftEnd = Carbon::parse($shift->shift_end); //end of employees shift
        $shiftStart = Carbon::parse($shift->shift_start); //begining of employee shift

        //This is the root of all evil. !!!
        $timeOut = Carbon::parse($log->ADate); 
    ...
    //dd("time calc in->Out difference " ,$timeIn,$timeOut, $imecalc/60 );
