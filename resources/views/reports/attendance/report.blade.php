...
<table cellpadding="5" cellspacing="0"{{ $hasWidth ? ' width=185mm' : '' }}>
    ...
    @foreach($attendances->groupBy('employee_id') as $attendanceGroup)
        ...
        <tr style="background-color:#e5e5e5;">
            ...
            <td style="text-align: right;"><strong>EXCESS HOURS</strong></td>
            <td style="text-align: right;"><strong>LESS HOURS</strong></td>      
        </tr>
        ...
        $diff = ($currentMins/60) - $attendance->shifthours;
        ?>
        ...
        <tr{{ $rowNumber % 2 == 0 ? ' style=background-color:#e5e5e5;' : '' }}>
            ...
            <td>{{ number_format($currentMins/60, 2) }}
            </td>
            <td>{{ $diff > 0 ? number_format($diff, 2) : 0 }}
            
            </td>
            <td>{{ $diff <= 0 ? number_format(-1 * ($diff), 2) : 0 }}</td>
