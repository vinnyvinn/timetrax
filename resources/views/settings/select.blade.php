            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <form action="{{ URL::current() }}/update" method="post" class="form-horizontal">
                        ...
                        <div class="form-group">
                            <div class="col-sm-12">
                                <select name="{{ $setting->setting_key == 'working_days' ? 'working_days[]' : $setting->setting_key }}" id="setting-select" class="form-control" {{ $setting->setting_key == 'working_days' ? 'multiple' : '' }}>
                                    @foreach($setting->options as $option)
                                        <option value="{{ strtolower($option) }}" {{ strtolower($option) == $setting->setting_value ? 'selected' : '' }}>{{ $option }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
